...
policy, _ := client.Policies.Create(
    ...
    map[string]any{
        ...
        "instructions": "Investigate the alert against each category. Use available data sources.",
        "dispositions": map[string]any{
            "classifications": []any{
                map[string]any{"name": "Suspicious", "description": "Activity warrants SAR filing"},
                map[string]any{"name": "Not Suspicious", "description": "Activity has legitimate explanation"},
                map[string]any{"name": "Needs Escalation", "description": "Requires senior analyst review"},
            },
        },
    },
...
agent, _ := client.Agents.Create(
    ...
    map[string]any{
        "policy_version_id": *policy.CurrentVersionID,
...
outputs, _ := client.Agents.RunSync(agent.ID, map[string]any{
...
Attach arbitrary metadata to any job when running an agent. Metadata is a dictionary of key-value pairs stored with the job, useful for tracking, filtering, or correlating jobs with your own internal records.
...
outputs, _ := client.Agents.RunSync("agent-uuid", map[string]any{
    "url": "https://example.com",
}, map[string]any{
    "batch":    "2026-02-12",
    "priority": "high",
